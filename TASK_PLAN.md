...
- ✅ **T8** — selection clears on delete/death/map-switch + occupancy drop-guard + 4px drag threshold + fittedRef-reset (no remount) + hover-race fix + tracked tween cancel + keyboard move/delete (D1–D7,D10); AC4 prevented-by-construction; D13 dropped. 64→68 tests
- ⬜ **T9** (next) → T10 → T11 → T12 → T13 → T14 → T15
